...
function readInputFile(inputFile::String)
    ...
    contrainte_droite=zeros(Int32, nb_lignes)
    contrainte_bas=zeros(Int32, nb_lignes)
    i=1
    for ligne in Data[2:nb_lignes+1]
        #print(i)
        ...
        contrainte_droite[i]=parse(Int64,ligneSepare[nb_lignes+1])
    i=i+1
    end

    ligneSepare=split(Data[nb_lignes+2],",")
    for i in 1:nb_lignes
        contrainte_bas[i]=parse(Int64,ligneSepare[i])
    end 

    return A, contrainte_droite, contrainte_bas
end

function displayGrid(A::Matrix,cont_d::Vector, cont_b::Vector )
    n, m = size(A)
    for i in 1:n
        for j in 1:m
            c = "."
...
function displaySolution(vX::Matrix)
    n, m = size(vX)
    for i in 2:n-1
        for j in 2:m-1
            c = "."
