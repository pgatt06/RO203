...
function readInputFile(inputFile::String)
    ...
    nb_lignes=parse(Int64,Data[1])

    A = Matrix{Int64}(undef, nb_lignes,nb_lignes)
    # Parcourir les lignes de données et remplir la matrice
    #compteur pour remplir les lignes correspondantes 

    i=1
    for ligne in Data[2:2+nb_lignes-1]
        #print(i)
        ligneSepare=split(ligne,",")
        
        for j in 1:nb_lignes
            A[i,j]=parse(Int64,ligneSepare[j])
        end
    i=i+1
    end
    #définition de la matrice des chemins possibles de taille line^2xlines^2
    #représente les chemins possibles entre chaque cases (indiqué par les flèches de la matrice A )
    CheminsPossibles=zeros(Int64, nb_lignes^2,nb_lignes^2)

    for i in 1:nb_lignes
        for j in 1:nb_lignes

            n_case=(i-1)*nb_lignes+j #car on numérote les cases de gauche à droite et de haut vers le bas 

            #définition des chemins existants pour tous les types de flèche 
            #chaque orientation est associée à un nombre 


            #flèche vers le haut associée à 0 
            if A[i,j]==0 
                k=1

                #condition de sortie du damier de jeu 
                while (n_case-k*nb_lignes>=1)
                    #on dit que le chemin etre la case n_case et toutes celles qui sont au dessus existe 
                    CheminsPossibles[n_case,n_case-k*nb_lignes]=1
                    k=k+1
                end

            #flèche vers le haut droit associé à 1 

            elseif A[i,j] == 1
                k=1 
                while (k<= min(nb_lignes-j,i-1))
                    CheminsPossibles[n_case,n_case-k*(nb_lignes-1)] = 1
                    k=k+1
                end
            ...
            elseif A[i,j] == 2
                k=1
                while(k<=nb_lignes-n_case%nb_lignes)
                    CheminsPossibles[n_case,n_case+k]=1
                    k=k+1
                end
        
            
            #flèche vers la droite et le bas 
            elseif A[i,j] == 3
                k=1 
                while (k<= min(nb_lignes-j,nb_lignes-i))
                    CheminsPossibles[n_case,n_case+k*(nb_lignes+1)] = 1
                    k=k+1
                end
            ...
            elseif A[i,j] == 4
                k = 1
                while  n_case+k*nb_lignes <= nb_lignes^2
                    CheminsPossibles[n_case,n_case+k*nb_lignes] = 1
                    k=k+1
                end
            
            #flèche vers le bas gauche 
            elseif A[i,j] == 5
                k =1 
                while (k<= min(nb_lignes-i,j-1))
                    CheminsPossibles[n_case,n_case+k*(nb_lignes-1)] = 1
                    k=k+1
                end
            ...
            elseif A[i,j] == 6
                k=1
                while (k<=(n_case-1)%nb_lignes)
                    CheminsPossibles[n_case,n_case-k] = 1
                    k=k+1
                end
            
            #flèche vers le haut gauche 
            elseif A[i,j] == 7
                k = 1
                while (k <= min(j-1,i-1))
                    CheminsPossibles[n_case,n_case-k*(nb_lignes+1)] = 1
    ...
    #définition de la matrice des contraintes 
    #elle permet d'imposer le départ et l'arrivée mais aussi d'imposer les cases intermédiaires
    Contraintes_jeux=Matrix{Int64}(undef,nb_lignes,nb_lignes)

    i=1
    for ligne in Data[2+nb_lignes+1:2*(nb_lignes+1)]
        ligneSerpare= split(ligne, ",")
        #print(i)
        for j in 1:nb_lignes
           Contraintes_jeux[i,j] = parse(Int64,ligneSerpare[j])
        end
        i=i+1
    end


    return A, CheminsPossibles,Contraintes_jeux
end
...
"""
    ...
    # On each page an array will contain at most maxInstancePerPage lignes with results
