...
function readInputFile(inputFile::String)
    ...
    list_top=[]
    for elt in elts
        push!(list_top,parse(Int,elt))
    end
    list_left=[]
    list_right=[]
    list_bottom=[]
    tableau= zeros(Int, n, k)
    for i in 3:(k+2)
        line =data[i]
        elts=split(line,",")
        push!(list_left,parse(Int,elts[1]))
        println(elts)
        println(n)
        push!(list_right,parse(Int,elts[n+2]))
    ...
    elts=split(line3,",")
    for elt in elts
        push!(list_bottom,parse(Int,elt))
    end

    return g,v,z,n,k,tableau,list_bottom,list_left,list_top,list_right
